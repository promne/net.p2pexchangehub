...
public class BufferedMongoContainer<Bean> extends MongoContainer<Bean>
    ...
    private final LinkedHashMap<Object,BeanItem<Bean>> newItems = new LinkedHashMap<>();
    private final LinkedHashMap<Object,BeanItem<Bean>> updatedItems = new LinkedHashMap<>();
    private final LinkedHashMap<Object,BeanItem<Bean>> removedItems = new LinkedHashMap<>();
    private final TreeSet<Integer> removedItemsIndices = new TreeSet<>();
    ...
    private void clearBuffers() {
        for (LinkedHashMap<Object,BeanItem<Bean>> temporaryStorage:
    ...
    private void commitRemovedItems() {
        for (Object key: removedItems.keySet()) {
    ...
    private void commitUpdatedItems() {
        for (Object key: updatedItems.keySet()) {
    ...
    private void commitNewItems() {
        for (Object key: newItems.keySet()) {
    ...
    public BeanItem<Bean> getItem(Object itemId) {
        Object id = assertIdValid(itemId);
    ...
    public boolean removeItem(Object itemId) throws
        ...
        if (newItems.containsKey(itemId)) {
            ...
            removedItems.put(itemId, super.getItem(itemId));
    ...
    public int indexOfId(Object itemId) {
        if (newItems.containsKey(itemId)) {
            int i = 0;
            for (Object newId: newItems.keySet()) {
    ...
    protected void fetchPage(int offset, int pageSize) {
        ...
        for (   ; cursor.hasNext() && index < newPage.maxIndex; ) {
            Object objectId = cursor.next().get(ID);
        ...
        if ( ( newPage.maxIndex >= super.size() - removedItems.size() )
            ...
            for (Object objectId: newItems.keySet()) {
     ...
     */
    public BeanItem<Bean> updateItem(Object itemId) {
    ...
    /**
     * Instantiate an entity automatically and return its Object
     *
    ...
    public Object addEntity(Bean target) {
        BeanItem<Bean> beanItem = new BeanItem<>(target, simpleProperties.keySet());
        for (String nestedProp: this.nestedProperties.keySet())
            beanItem.addNestedProperty(nestedProp);

        Object id = beanFactory.injectId(target);
